...
#
# Two of the five measured systems (DeepProve, jolt-atlas) are licensed for
...
while IFS= read -r f; do
  if head -20 "$f" | grep -qiE 'copyright.*(lagrange|icme|scroll|consensys|irreducible)'; then
...
for d in systems/binius64 systems/deepprove systems/jolt-atlas systems/ceno systems/gnark; do
  [ -d "$d" ] || { echo "MISSING OUR OWN DOCS: $d" >&2; fail=1; }
done

[ "$fail" -eq 0 ] && echo "OK: no vendored third-party source; all five system directories present"
exit "$fail"
